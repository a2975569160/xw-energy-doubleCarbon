...
define(['./when-54c2dc71', './Check-6c0211bc', './Math-850675ea', './Cartesian2-ea28baad', './Transforms-2e98bea0', './RuntimeError-2109023a', './WebGLConstants-76bb35d1', './ComponentDatatype-a26dd044', './AttributeCompression-8a649799', './IntersectionTests-d561f371', './Plane-1577f476', './WebMercatorProjection-75bbddfc', './createTaskProcessorWorker', './EllipsoidTangentPlane-fc24177b', './OrientedBoundingBox-765f5105', './TerrainEncoding-adb973b4'], function (when, Check, _Math, Cartesian2, Transforms, RuntimeError, WebGLConstants, ComponentDatatype, AttributeCompression, IntersectionTests, Plane, WebMercatorProjection, createTaskProcessorWorker, EllipsoidTangentPlane, OrientedBoundingBox, TerrainEncoding) { 'use strict';

  /**
   * Contains functions to create a mesh from a heightmap image.
   *
   * @namespace CgsHeightmapTessellator
   *
   * @private
   */
  var CgsHeightmapTessellator = {};

  /**
   * The default structure of a heightmap, as given to {@link CgsHeightmapTessellator.computeVertices}.
   *
   * @constant
   */
  CgsHeightmapTessellator.DEFAULT_STRUCTURE = Object.freeze({
    heightScale: 1.0,
    heightOffset: 0.0,
    elementsPerHeight: 1,
    stride: 1,
   ...
   * var width = 5;
   * var height = 5;
   * var statistics = Cesium.CgsHeightmapTessellator.computeVertices({
   ...
   */
  function isPolygonInside(clipPoint, node) {
    var inside = false;
    var length = clipPoint.length;
    for (var m = 0, n = length - 1; m < length; n = m, ++m) {
      var node0 = clipPoint[n];
      var node1 = clipPoint[m];
      if (node.y < node1.y) {
        if (node.y >= node0.y) {
          if ((node.y - node0.y) * (node1.x - node0.x) > (node.x - node0.x) * (node1.y - node0.y)) {
            inside = !inside;
          }
        }
      }
      else if (node.y < node0.y) {
        if (node.y >= node1.y) {
          if ((node.y - node0.y) * (node1.x - node0.x) < (node.x - node0.x) * (node1.y - node0.y)) {
            inside = !inside;
          }
  ...
  CgsHeightmapTessellator.computeVertices = function (options) {
    ...
    var structure = when.defaultValue(
      options.structure,
      CgsHeightmapTessellator.DEFAULT_STRUCTURE
    );
    var heightScale = when.defaultValue(
      structure.heightScale,
      CgsHeightmapTessellator.DEFAULT_STRUCTURE.heightScale
    );
    var heightOffset = when.defaultValue(
      structure.heightOffset,
      CgsHeightmapTessellator.DEFAULT_STRUCTURE.heightOffset
    );
    var elementsPerHeight = when.defaultValue(
      structure.elementsPerHeight,
      CgsHeightmapTessellator.DEFAULT_STRUCTURE.elementsPerHeight
    );
    var stride = when.defaultValue(
      structure.stride,
      CgsHeightmapTessellator.DEFAULT_STRUCTURE.stride
    );
    var elementMultiplier = when.defaultValue(
      structure.elementMultiplier,
      CgsHeightmapTessellator.DEFAULT_STRUCTURE.elementMultiplier
    );
    var isBigEndian = when.defaultValue(
      structure.isBigEndian,
      CgsHeightmapTessellator.DEFAULT_STRUCTURE.isBigEndian
    );
    ...
    var isTerrain = when.defaultValue(options.isTerrain, false);
    var clipCount = options.clipRegions ? options.clipRegions.length : 0;
    ...
    for (var rowIndex = startRow; rowIndex < endRow; ++rowIndex) {
      ...
      for (var colIndex = startCol; colIndex < endCol; ++colIndex) {
        ...
        //by start HGT: 瓦片网格各位置顶点及高程,处理地形裁剪  20180821
        var llh = Cartesian2.Cartographic.fromCartesian(position);
        var nodePosition = new Cartesian2.Cartesian3(llh.longitude, llh.latitude, llh.height);
        for (var m = 0; m < clipCount; ++m) {
          var clipRegion = options.clipRegions[m];
          var regionPositions = clipRegion.positions;
          var factor = clipRegion.factor;
          var terrainMode = clipRegion.terrainMode;
          var minAlt = clipRegion.minAlt;
          var maxAlt = clipRegion.maxAlt;
          if (isPolygonInside(regionPositions, nodePosition)) {
            isProcessed = true;
            //地形下沉
            if (terrainMode === 1) {
              llh.height -= Math.abs(factor);
              var cartesian = Cartesian2.Cartesian3.fromRadians(llh.longitude, llh.latitude, llh.height);
              Cartesian2.Cartesian3.clone(cartesian, position);
            } else if (terrainMode === 2) {//地形夸张
              if (isTerrain) {
                if (exaggeration === 1.0) {
                  var h;
                  var deltaA = llh.height - minAlt;
                  h = llh.height + deltaA * (factor - 1);
                  llh.height = h;
                  var cartesian = Cartesian2.Cartesian3.fromRadians(llh.longitude, llh.latitude, llh.height);
                  Cartesian2.Cartesian3.clone(cartesian, position);
                }
              }
            } else if (terrainMode === 3) {//地形推平
              llh.height = factor;
              var cartesian = Cartesian2.Cartesian3.fromRadians(llh.longitude, llh.latitude, llh.height);
              Cartesian2.Cartesian3.clone(cartesian, position);
            } else {//地形拉升
  ...
  var cartesian3Scratch$1 = new Cartesian2.Cartesian3();
  var matrix4Scratch$1 = new Transforms.Matrix4();
  var minimumScratch$1 = new Cartesian2.Cartesian3();
  var maximumScratch$1 = new Cartesian2.Cartesian3();
  ...
  HeightmapTessellator.computeVertices = function (options) {
    ...
    var fromENU = Transforms.Transforms.eastNorthUpToFixedFrame(relativeToCenter, ellipsoid);
    var toENU = Transforms.Matrix4.inverseTransformation(fromENU, matrix4Scratch$1);
    ...
    var minimum = minimumScratch$1;
    minimum.x = Number.POSITIVE_INFINITY;
    minimum.y = Number.POSITIVE_INFINITY;
    minimum.z = Number.POSITIVE_INFINITY;

    var maximum = maximumScratch$1;
    maximum.x = Number.NEGATIVE_INFINITY;
    maximum.y = Number.NEGATIVE_INFINITY;
    maximum.z = Number.NEGATIVE_INFINITY;
    ...
    for (var rowIndex = startRow; rowIndex < endRow; ++rowIndex) {
      ...
      for (var colIndex = startCol; colIndex < endCol; ++colIndex) {
        ...
        Transforms.Matrix4.multiplyByPoint(toENU, position, cartesian3Scratch$1);

        Cartesian2.Cartesian3.minimumByComponent(cartesian3Scratch$1, minimum, minimum);
        Cartesian2.Cartesian3.maximumByComponent(cartesian3Scratch$1, maximum, maximum);
        hMin = Math.min(hMin, heightSample);
      }
    }
  ...
  */
  ...
  function createVerticesFromHeightmap(parameters, transferableObjects) {
    ...
    var statistics = CgsHeightmapTessellator.computeVertices(parameters);//mgj
    ...
    return {
      ...
      isProcessed: statistics.isProcessed,//mgj
    };
  }
