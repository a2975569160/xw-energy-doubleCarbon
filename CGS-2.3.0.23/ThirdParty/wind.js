...
var Windy = function Windy(params) {
	//20180408 chen 增加支持从外部传绘制颜色，若没传则默认使用内部根据温度计算的颜色
	//支持传入绘制的流线的条数
	//支持传入单条线最多延续的次数
	//start
	this.fillColor = params.fillColor;
	this.particleCount = params.lineCount;
	this.maxParticleAge = params.speed;
	var that = this;
	//end
	...
	var createWindBuilder = function createWindBuilder(uComp, vComp, temp) {
		...
		return {
			...
			data: function data(i) {
				//20180408 chen处理当没有温度数据的情况，此时默认温度为0度
				//start
				//return [uData[i], vData[i], temp.data[i]];
				if(temp){
					return [uData[i], vData[i], temp.data[i]];
				}else{
					return [uData[i], vData[i], 0];
				}
				//end
  ...
  */
   ...
   */
	...
	var animate = function animate(bounds, field, extent) {
		...
		function draw() {
			...
			buckets.forEach(function (bucket, i) {
				if (bucket.length > 0) {
					g.beginPath();
					//20180408 chen 增加支持从外部传绘制颜色，若没传则默认使用内部计算的颜色
					//start
					//g.strokeStyle = colorStyles[i];
					if(that.fillColor){
						g.strokeStyle = that.fillColor;
					}else{
						g.strokeStyle = colorStyles[i];
					}
					//end
	...
	var start = function start(bounds, width, height, extent) {
		...
		stop();
		//20180410 chen 支持动态设置单条线最多的延续次数
		//start
		if(that.maxParticleAge){
			MAX_PARTICLE_AGE = that.maxParticleAge;
		}
		//end
		// build grid
	...
	//20180410 chen 增加设置接口
	//start
	var setFillColor = function(fillColor){
		that.fillColor = fillColor;
	}
	var setParticleCount = function(lineCount){
		that.particleCount = lineCount;
	}
	var setMaxParticleAge = function(speed){
		that.maxParticleAge = speed;
	}
	//end
	var windy = {
		params: params,
		...
		setMaxParticleAge: setMaxParticleAge,
		//end
	};
