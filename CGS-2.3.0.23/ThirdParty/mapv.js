(function (global, factory) {
	...
	(factory((global.CgsMapV = global.CgsMapV || {})));
...
DataSet.prototype.get = function (args) {
    ...
    if (args.transferCoordinate && data.length != 0) {//mgj
...
var pathSimple = {
    ...
    drawPolygon: function drawPolygon(context, coordinates) {
        ...
        for (var k = 0; k < coordinates.length; k++) {
            ...
            var newCoordinate = [];
            for (var l = 0; l < coordinate.length; l++) {
                if(coordinate[l][0] == -1 && coordinate[l][1] == -1){
                    continue;
                }
                newCoordinate.push(coordinate[l]);
            }
...
//mgj
/*
function filterFn(data, map){
    function getExtent() {
        var extent = map.camera.computeViewRectangle(map.scene.globe.ellipsoid);
        var result = {
    ...
    var west = WESN.west;
    var east = WESN.east;
    var south = WESN.south;
    var north = WESN.north;

    var geometry = data.geometry;
    var type = geometry.type;
    if(type == 'MultiPolygon'){
        var multiCoordinates = geometry.coordinates;
        var newMultiCoordinates = [];
        for(var i = 0; i < multiCoordinates.length; ++i){
            var coordinates = multiCoordinates[i];
            var newCoordinates = [];
            for(var j = 0; j < coordinates.length; ++j){
            ...
            newMultiCoordinates.push(newCoordinates);
        }
        data.geometry.coordinates = newMultiCoordinates;
        ...
        data.geometry.coordinates = [newCoordinates];
        ...
        var coordinates = geometry.coordinates;
        var newCoordinates = [];
        if((coordinates[0] > west && coordinates[0] < east) && (coordinates[1] > south && coordinates[1] < north)){
...
var drawSimple = {
    draw: function draw(context, dataSet, options, map) {
        // debugger;

        var data = dataSet instanceof DataSet ? map instanceof CgsMap3D ? dataSet.get({filter: computeVisible, map: map}) : dataSet.get() : dataSet;//mgj
        // var data = dataSet instanceof DataSet ? dataSet.get() : dataSet;
        //mgj
        if(data.length == 0){
            return;
...
//mgj
function computeVisible(data, map) {
   
    var geometry = data.geometry;
    var type = geometry.type;
    if(type == 'Point'){
        var coordinates = geometry.coordinates;
        return computeVisible_(coordinates, map);
    }else if(type == 'MultiPolygon'){
        var multiCoordinates = geometry.coordinates;
        var _multiCoordinates = geometry._coordinates;
        var newMultiCoordinates = [];
        var _newMultiCoordinates = [];
        for(var i = 0; i < multiCoordinates.length; ++i){
            var coordinates = multiCoordinates[i];
            var _coordinates = _multiCoordinates[i];
            var newCoordinates = [];
            var _newCoordinates = [];
            for(var j = 0; j < coordinates.length; ++j){
                var coords = coordinates[j];
                var _coords = _coordinates[j];
                var newCoords = [];
                var _newCoords = [];
                for(var k = 0; k < coords.length; ++k){
                    var flag = computeVisible_(coords[k], map);
                    if(flag){
                        newCoords.push(coords[k]);
                        _newCoords.push(_coords[k]);
                    }
                }
                if(newCoords.length != 0){
                    newCoordinates.push(newCoords);
                    _newCoordinates.push(_newCoords);
                }
            }
            if(newCoordinates.length != 0){
                 newMultiCoordinates.push(newCoordinates);
                _newMultiCoordinates.push(_newCoordinates);
            }
        }
        data.geometry.newCoordinates = newMultiCoordinates;
        data.geometry._coordinates = _newMultiCoordinates;
    }else if(type == 'Polygon'){
        var coordinates = geometry.coordinates[0];
        var _coordinates = geometry._coordinates[0];
        var newCoordinates = [];
        var _newCoordinates = [];
        for(var j = 0; j < coordinates.length; ++j){
            var flag = computeVisible_(coordinates[j], map);
            if(flag){
                newCoordinates.push(coordinates[j]);
                _newCoordinates.push(_coordinates[j]);
            }
        }
        if(newCoordinates.length == 0){
            data.geometry.newCoordinates = [];
            data.geometry._coordinates = [];
        }else{
            data.geometry.newCoordinates = [newCoordinates];
            data.geometry._coordinates = [_newCoordinates];
        }
        
    }else if(type == 'LineString'){
        var coordinates = geometry.coordinates;
        var _coordinates = geometry._coordinates;
        var newCoordinates = [];
        var _newCoordinates = [];
        for(var j = 0; j < coordinates.length; ++j){
            var flag = computeVisible_(coordinates[j], map);
        ...
        for(var i = 0; i < multiCoordinates.length; ++i){
            ...
            for(var j = 0; j < coordinates.length; ++j){
                ...
                var _newCoords = [];
                    var flag = computeVisible_(coordinates[j], map);
                    if(flag){
                        newCoords.push(coordinates[j]);
                        _newCoords.push(_coordinates[j]);
                    }
        ...
        data.geometry.newCoordinates = multiCoordinates;
        data.geometry._coordinates = _multiCoordinates;
    }
    if(data.geometry.newCoordinates.length == 0){
        return false;
    }else{
        return true;
    }
}
function computeVisible_(coordinates, map) {
    var rX = 6378137.0;
    var rY = 6378137.0;
    var rZ = 6356752.3142451793;

    //规范化
    var cameraPosition = map.camera.position;
    ...
    var position = coordinates.slice(0);
    if (!position) {
        return false;
    }
    if (position[2] == null) {
    ...
    var cartesianPosition = Cesium.Cartesian3.fromDegrees(position[0], position[1], position[2]);
    var tX = cartesianPosition.x / rX;
    var tY = cartesianPosition.y / rY;
    var tZ = cartesianPosition.z / rZ;
...
var drawHoneycomb = {
    draw: function draw(context, dataSet, options, map) {//mgj
...
var MapHelper = function () {
    ...
    createClass(MapHelper, [{
        ...
        value: function addLayer(datas, options) {
            if (this.type == 'baidu') {
                return new CgsMapV.baiduMapLayer(this.map, dataSet, options);
            }
        }
    }, {
        key: 'getMap',
...
var drawText = {
    draw: function draw(context, dataSet, options, map) {//mgj
        ...
        //mgj
        var data = dataSet instanceof DataSet ? map instanceof CgsMap3D ? dataSet.get({filter: computeVisible, map: map}) : dataSet.get() : dataSet;
        //var data = dataSet instanceof DataSet ? dataSet.get() : dataSet;
        //end
        context.save();
            ...
            switch (self.options.draw) {
                ...
                case 'honeycomb':
                    ...
                    if (self.options.draw === 'grid') {
                        ...
                        drawHoneycomb.draw(context, dataSet, self.options, self.map);//mgj
                    }
                    break;
                case 'text':
                    drawText.draw(context, dataSet, self.options, self.map);//mgj
                    break;
                case 'icon':
                ...
                default:
                    if (self.options.context == "webgl") {
                        ...
                        drawSimple.draw(context, dataSet, self.options, self.map);//mgj
                    }
            }
...
var Layer$8 = function (_BaseLayer) {
  ...
  createClass(Layer, [{
    key: "init",
    value: function init(map, options) {
      if (map && map instanceof CgsMap2D) {//mgj
        this.$Map = map;
        this.context = this.options.context || '2d';
        this.getCanvasLayer();
        this.initDataRange(options);
    ...
    value: function render(canvas, time) {
      ...
      var dataGetOptions = {
        transferCoordinate: function transferCoordinate(coordinate) {
          return map.getPixelFromCoordinate(ol.proj.transform(coordinate, 'EPSG:4326', _projection));//mgj
        }
      };
...
var MapVRenderer$1 = function (_BaseLayer) {
    ...
    createClass(MapVRenderer, [{
        ...
        value: function init(t) {
            this.options = t, this.initDataRange(t), this.context = this.options.context || "2d", this.options.zIndex && this.canvasLayer && this.canvasLayer.setZIndex && this.canvasLayer.setZIndex(this.options.zIndex), this.initAnimator();//mgj
        }
    }, {
        key: "_canvasUpdate",
...
var geojson = {
    ...
    //mgj
    getData: function getData(geoJson) {
